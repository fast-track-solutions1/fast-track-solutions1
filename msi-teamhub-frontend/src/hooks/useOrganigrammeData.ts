...
export interface Salarie {
  ...
  service: number;      // ✅ champ venant de l'API: "service"
  grade: number;        // ✅ champ venant de l'API: "grade"
  salaire?: number;
  en_poste?: boolean;
}
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Gérer pagination {results: [...]} ou tableau direct
        ...
        console.log('   Total employés placés:', countTotalEmployees(hierarchy));
...
function buildHierarchy(
  ...
  // Associer TOUS les salariés à leurs services + grade
  salaries.forEach((salarie) => {
    const grade = gradesMap.get(salarie.grade);
    if (!grade) {
      console.warn(
        `⚠️  Grade ${salarie.grade} non trouvé pour salarié ${salarie.id}`
      );
      return;
    }
  ...
  // Trier les salariés par ordre de grade
  ...
  // Racines = services SANS parent_service (null, undefined, ou 0)
  const rootServices = services.filter(
    (s) =>
      s.parent_service === null ||
      s.parent_service === undefined ||
      s.parent_service === 0
  );

  console.log(
    '🌱 Services racines trouvés:',
    rootServices.length,
    rootServices.map((s) => s.nom)
  );
 ...
 */
export function flattenServiceHierarchy(
  nodes: ServiceNode[]
): ServiceEmployee[] {
