...
export default function AnnuaireServicesPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const endpoint = `${apiUrl}/api/services/?limit=1000`
  ...
  const filteredAndSortedServices = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter(s =>
        ...
        (s.societe?.nom && s.societe.nom.toLowerCase().includes(search))
      )
    }
    ...
    filtered.sort((a, b) => {
      ...
      if (sortField === 'nom') {
        ...
        comp = (a.societe?.nom || '').localeCompare(b.societe?.nom || '')
      }
      return sortOrder === 'asc' ? comp : -comp
    })

    return filtered
  }, [services, searchTerm, filterActif, sortField, sortOrder])

  const stats = useMemo(() => ({
    total: services.length,
    actives: services.filter(s => s.actif).length,
    inactives: services.filter(s => !s.actif).length,
    resultats: filteredAndSortedServices.length,
  ...
  const handleExportCSV = useCallback(() => {
    ...
    const headers = ['Nom', 'Description', 'Société', 'Statut', 'Date de création']
    const rows = filteredAndSortedServices.map(s => [
      s.nom,
      s.description || '',
      s.societe?.nom || '',
    ...
    link.download = `annuaire_services_${new Date()
      .toISOString()
      .split('T')[0]}.csv`
    link.click()
  }, [filteredAndSortedServices])

  const handleSort = useCallback((field: 'nom' | 'societe') => {
    if (sortField === field) {
  ...
  return (
    <div className="flex flex-col gap-6 p-6 bg-white dark:bg-slate-950 min-h-screen">
      ...
      {filteredAndSortedServices.length === 0 && !loading && !error && (
                ...
                <div className="space-y-2 text-sm text-slate-600 dark:text-slate-400">
                  {service.description && (
                    <p className="line-clamp-2">
                      <span className="font-medium">📝</span> {service.description}
                    </p>
                  )}
                  {service.societe && (
                    <p>
                      <span className="font-medium">🏢</span> {service.societe.nom}
                    </p>
                  )}
                </div>
